...
from dataloader import DataLoader  # ← добавь обратно после time_utils
...
def load_dataset(args, data_format):
    data_path = os.path.join('/home/goha/DocumentsG/Goha/Goha/data/AIS', 'AIS_EAST')
    if data_format == 'csv':
        ...
        id2loc = pickle.load(open(os.path.join(data_path, 'grid2center_AIS_WEST.pickle'), 'rb'))
...
def validation(dataset, model, A, device):
    ...
    for i, batch_data in enumerate(dataset):
        with torch.no_grad():
            ...
            outputs = model(batch_enc_loc, batch_enc_time,batch_enc_time_segments, batch_enc_coor, src_mask, A, 'tagging')
