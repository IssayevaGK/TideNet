...
def train_tagging(args):
    ...
    train_dataset = TrajectoryTaggingDataset(train_data, args, max_len, drop_num=[1,2,3,4], drop_ratio=[0.2,0.4,0.6,0.7], id2loc=id2loc)
    ...
    for epoch in range(args.num_epochs):
        ...
        for iteration, (batch_data, batch_cl) in enumerate(train_dataloader):
            ...
            batch_enc_loc, batch_enc_time,batch_enc_time_segments, batch_enc_coor, batch_lengths, batch_target = batch_data
            batch_cl_loc, batch_cl_time,batch_cl_time_segments, batch_cl_coor, batch_cl_lengths = batch_cl
            ...
            pred = detection_model(batch_enc_loc, batch_enc_time,batch_enc_time_segments, batch_enc_coor, src_mask, A, 'tagging')

            optimizer.zero_grad()

            if epoch >= args.warm_up_epochs:
                loss_tagging = loss_func(pred.view(-1, args.num_cls), batch_target.view(-1), src_mask.squeeze(), ce_loss)
                cl_outputs = detection_model(batch_cl_loc, batch_cl_time, batch_cl_time_segments, batch_cl_coor, attn_mask_cl, A, 'contrastive')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ce_weight", type=float, default=5.0,
    ...
    parser.add_argument("--data_name", type=str, default="AIS_WEST",
                        help="data name")
    parser.add_argument("--segment_duration_hours", type=int, default=2, help="Duration of each time segment in hours")
    parser.add_argument("--num_time_segments", type=int, default=10, help="Number of time segments in 24 hours")
