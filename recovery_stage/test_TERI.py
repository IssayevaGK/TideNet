...
from time_utils import time_to_segment
...
def test_twostage(args):
    ...
    test_input, test_target, loc_size, id2loc, max_len, adj_graph, drop_ratios, num_labels, distance = load_test_dataset(
        args, data_path, adj_path)
    ...
    for i in range(num_iter):
        with torch.no_grad():
            ...
            traj_inp_loc, traj_inp_time,traj_inp_time_segments, traj_inp_coors = traj_inp[:, :, 0], traj_inp[:, :, 1:2],time_to_segment(traj_inp[:, :, 1]), traj_inp[:, :, 2:]
            ...
            traj_inp_time_segments = torch.tensor(traj_inp_time_segments, dtype=torch.long, device=args.device)
            ...
            outputs = tagging_model(traj_inp_loc, traj_inp_time,traj_inp_time_segments, traj_inp_coors, src_mask, A, 'tagging')
    ...
    inputs = []
    final_preds = []
    #if len(final_preds) == 0 or len(test_target) == 0:
    #    print("Final predictions or test targets are empty.")
    #    return
    ...
    for i in range(num_iter):
        with torch.no_grad():
            ...
            traj_inp = pad_arrays(traj_inp)
            traj_locs, traj_tms,traj_tms_segments,traj_coors = traj_inp[:, :, 0], traj_inp[:, :, 1:2],time_to_segment(traj_inp[:, :, 1]), traj_inp[:, :, 2:]
            ...
            traj_tms_segments = torch.tensor(traj_tms_segments, dtype=torch.long, device=args.device)
            ...
            for idx in range(masked_pos_batch.shape[1]):
                ...
                trg_probs = insertion_model(traj_locs, traj_tms,traj_tms_segments, traj_coors, attn_mask, A, 'recovery', batch_masked_pos_cur,
                                            batch_pred_inputs)
...
def evaluate(args, test_input, preds, test_target, num_labels, id2loc, maxlen, data_path):
    ...
    to4326 = Transformer.from_crs(f"epsg:{4575}", "epsg:4326", always_xy=True)
    ...
    for idx, (drop, pred, label_origin, tag) in enumerate(zip(test_input, preds, test_target, num_labels)):
        ...
        if len(pred) == len(tag):
            ...
            if single_RMSE_count != 0:
                ...
                if tmp_single_RMSE >= 0 and tmp_single_RMSE <= 1000:
                    RMSE += single_RMSE
...
def euclidean_square_distance(p1, p2):
    to4326 = Transformer.from_crs(f"epsg:{4575}", "epsg:4326")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='/home/goha/DocumentsG/Goha/Goha/segm/west/model/model_AIS_WEST_test',
                        help='Model path')
    parser.add_argument('--data_path', type=str, default='/home/goha/DocumentsG/Goha/Goha/data/AIS',
                        help='Dataset path')
    parser.add_argument("--data_name", type=str, default="AIS_WEST",
                        help="data name")
    parser.add_argument("--candidate_loc_distance", type=int, default=3000,
                        help="candidate loc distance")
    parser.add_argument("--time_step", type=str, default=4,
                        help="time_step")
    parser.add_argument("--segment_duration_hours", type=int, default=3, help="Duration of each time segment in hours")
    parser.add_argument("--num_time_segments", type=int, default=8, help="Number of time segments in 24 hours")
