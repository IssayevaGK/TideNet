...
class Transformer_insertion(nn.Module):
    ...
    def __init__(self, model_dimension, fourier_dimension, time_dimension,num_time_segments, src_vocab_size, trg_vocab_size, number_of_heads, number_of_layers,
    ...
    def forward(self, src_token_ids_batch, src_time_batch,src_time_batch_segments, src_dist_batch, attn_mask, adj_graph, type,
                masked_pos=None, src_pred_inputs_batch=None):
        src_representations_batch = self.encode(src_token_ids_batch, src_time_batch,src_time_batch_segments, src_dist_batch, attn_mask, adj_graph, type,
    ...
    def encode(self, src_token_ids_batch, src_time_batch, src_time_batch_segments, src_dist_batch, attn_mask, adj_graph, type,
        ...
        if type == 'recovery':
            ...
            times_batch_segments = torch.cat([
                src_time_batch_segments,
                torch.zeros(bs, pred_inputs_batch.shape[1], dtype=torch.long, device=self.device)
            ], dim=1)
            ...
            times_batch_segments = src_time_batch_segments
        ...
        if self.learnable_pos:
            ...
            time_embeddings_batch_segments = self.time_embedding_segments(times_batch_segments)
            ...
            embeddings_batch = token_embeddings_batch + pos_embeddings_batch + time_embeddings_batch + time_embeddings_batch_segments + dist_embeddings_batch
        else:
