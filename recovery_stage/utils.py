...
def load_dataset(args, data_format):
    data_path = os.path.join('/home/goha/DocumentsG/Goha/Goha/data/AIS', 'AIS_WEST')
    if data_format == 'csv':
...
def validation(dataset, model, A, device, sample=False):
    ...
    for i, batch_data in enumerate(dataset):
        with torch.no_grad():
            ...
            batch_loc, batch_time,batch_time_segments, batch_coor, batch_lengths, batch_masked_pos, batch_masked_pos_lengths = batch_data
            ...
            for idx in range(batch_masked_pos.shape[1]):
                ...
                trg_probs = model(batch_loc, batch_time,batch_time_segments, batch_coor, attn_mask, A, 'recovery',
