...
def dataloader_collate_test(batch):
    ...
    trip_locs = src[:, :, 0]
    trip_tms = src[:, :, 1:2]
    trip_tms_segments = time_to_segment(src[:, :, 1])
    trip_coors = src[:, :, 2:]
    ...
    res_tensors = (
        ...
        torch.tensor(trip_tms_segments, dtype=torch.long),
...
def dataloader_collate(batch):
    ...
    trip_cl_tms_segments = time_to_segment(src_cl_pairs[:, :, 1])
    ...
    cl_tensors = (
        ...
        torch.tensor(trip_cl_tms_segments, dtype=torch.long),
